...
def generator(x):
    ...
    # 2st hidden layer
    w2 = tf.get_variable('G_w0', [h0.get_shape()[1], 512], initializer=w_init)
    b2 = tf.get_variable('G_b0', [512], initializer=b_init)
    h2 = tf.nn.relu(tf.matmul(h0, w2) + b2)

    # 3st hidden layer
    w3 = tf.get_variable('G_w0', [h2.get_shape()[1], 1024], initializer=w_init)
    b3 = tf.get_variable('G_b0', [1024], initializer=b_init)
...
def discriminator(x, drop_out):
    ...
    h0 = tf.nn.relu(tf.matmul(x, w0) + b0)
    h0 = tf.nn.dropout(tf.matmul(h0))

    # 2st hidden layer
    w2 = tf.get_variable('D_w0', [h0.get_shape()[1], 512], initializer=w_init)
    b2 = tf.get_variable('D_b0', [512], initializer=b_init)
    h2 = tf.nn.relu(tf.matmul(h0, w2) + b2)
    h2 = tf.nn.dropout(tf.matmul(h2))

    # 3st hidden layer
    w3 = tf.get_variable('D_w0', [h2.get_shape()[1], 512], initializer=w_init)
    b3 = tf.get_variable('D_b0', [512], initializer=b_init)
    h3 = tf.nn.relu(tf.matmul(h2, w3) + b3)
    h3 = tf.nn.dropout(tf.matmul(h3))




    # output layer
    w1 = tf.get_variable('D_w1', [h3.get_shape()[1], 1], initializer=w_init)
    b1 = tf.get_variable('D_b1', [1], initializer=b_init)
    o = tf.sigmoid(tf.matmul(h3, w1) + b1)
    ###  Code: ToDO( Change the architecture as CW2 Guidance required)
